class ClassroomsController < ParsingController
  ...
  def eclass
    ...
    @eclass_content_list.each do |content| #get_eclass_notice_list 함수로 불러오는 chunk에는 lectureno 값이 없다. presenters_controller.rb/notice_detail 을 보게 되면 공지사항 글 세부 조회를 위해서 lectureno가 필요한데 결과 chunk에 없으므로 다시 [0~N][2]자리에 course_number 값을 넣어준다.
